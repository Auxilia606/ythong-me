...
If a file appears corrupted or unreadable, do not delete, rewrite, or clean up the suspicious parts immediately. First check the encoding state, reload the file explicitly with the expected encoding, and only confirm what is actually stored.
